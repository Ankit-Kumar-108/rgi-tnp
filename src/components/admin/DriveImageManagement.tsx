...
export default function DriveImageManagement() {
  ...
  const fetchImages = useCallback(async () => {
    try {
      let url = `/api/admin/drive-images?page=${page}&limit=20`;
    ...
    const check = async () => {
      try {
        const res = await fetch(
          `/api/admin/drive-images?driveId=${selectedDrive}&limit=100`,
        );
        const data = (await res.json()) as {
          success: boolean;
          images: DriveImage[];
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/drive-images", {
        method: "POST",
  ...
  const handleDelete = async (imageId: string) => {
    ...
    setDeleting(imageId);
    try {
      const res = await fetch(`/api/admin/drive-images/${imageId}`, {
        method: "DELETE",
      });
