...
import { BadgeCheck, BadgeAlert, Briefcase, CheckCircle, Camera, FileText, Loader2, Upload, Building2, User, Mail, Phone, User2, ExternalLink, Eye, X } from "lucide-react";
import { toast } from "sonner";
import { getToken } from "@/lib/auth-client";
import { uploadFileToR2 } from "@/lib/upload-r2";
import ExternalDashboardHeader from "./ExternalDashboardHeader";

interface ExternalDashboardOverviewProps {
  student: any;
  drivesLength: number;
...
export default function ExternalDashboardOverview({
  student,
  drivesLength,
  registrationsLength,
  loading,
  ...
  handleLogout,
}: ExternalDashboardOverviewProps) {
  ...
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const url = await uploadFileToR2(file, "resumes", { role: "external_student" });
            ...
            <div className="p-6 border-b border-border flex items-center justify-between bg-linear-to-r from-brand/5 to-transparent shrink-0">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  College: <span className="font-bold text-foreground">{student?.collegeName || "N/A"}</span>
                </p>
