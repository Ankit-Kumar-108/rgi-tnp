...
export default function RecruiterRegister() {
    ...
    const inputClass = "w-full pl-11 pr-12 py-3 sm:py-3.5 rounded-lg border border-input bg-background focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none transition-all placeholder:text-muted-foreground text-sm sm:text-base shadow-sm";
                ...
                <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 bg-background/50 backdrop-blur-sm rounded-lg  shadow-2xl shadow-brand/10 border border-brand/10 lg:h-[85vh] lg:max-h-200 mt-10 overflow-hidden">
                    <div className="relative hidden lg:flex flex-col justify-end p-10 xl:p-12 overflow-hidden bg-brand/5">
                        ...
                        <div className="relative z-10 text-white">
                            <div className="mb-4 inline-flex items-center justify-center p-3 bg-brand backdrop-blur-md rounded-lg text-brand-50 shadow-inner"><Briefcase className="w-8 h-8"/></div>
                            <h1 className="text-4xl font-black mb-4 leading-tight">Partner With Excellence</h1>
                            <p className="text-lg text-white/80 font-light max-w-md">Register your company to access a pool of talented, driven, and industry-ready professionals.</p>
                    ...
                    <div className="p-5 sm:p-8 md:p-10 lg:p-12 flex flex-col relative h-full lg:overflow-y-auto overflow-x-hidden">
                        ...
                        <div className="absolute bottom-0 left-0 -mb-16 -ml-16 w-32 h-32 rounded-full bg-brand/15 blur-2xl z-0 pointer-events-none"></div>
                        <div className="mb-6 relative z-10">
                            <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground mb-2">Company Registration</h2>
                            <p className="text-muted-foreground text-xs sm:text-sm">Register your organization to access our talent pool.</p>
                        </div>

                        {error && <div className="mb-3 p-3 rounded-lg bg-destructive/10 text-destructive text-sm font-medium relative z-10">{error}</div>}
                        {success && <div className="mb-3 p-3 rounded-lg bg-green-500/10 text-green-600 dark:text-green-400 text-sm font-medium relative z-10">{success}</div>}
                        ...
                        <div className="mt-6 pt-4 border-t border-border relative z-10">
                            ...
                            <Link href="/recruiters/login" className="w-full py-3 sm:py-3.5 rounded-lg border-2 border-brand/20 text-brand font-bold bg-transparent hover:bg-brand/5 transition-all duration-200 flex items-center justify-center gap-2">Login Here</Link>
                        </div>
                    </div>
                </div>
            </main>
