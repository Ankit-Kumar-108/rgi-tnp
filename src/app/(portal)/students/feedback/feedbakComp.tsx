...
const TAGS = ["Placements", "Curriculum", "Support", "UI/UX", "Events", "Other"];

export default function FeedbackComp({ onClose }: FeedbackCompProps = {}) {
  const { loading: authLoading, authenticated } = useAuth("student", "/student/register");
  ...
  if (submitted) {
    return (
      <div className="bg-card border border-border rounded-[2rem] p-6 md:p-12 text-center animate-in zoom-in-95 fade-in duration-300 shadow-xl w-full max-w-md mx-auto relative overflow-hidden">
    ...
    <div className="bg-card rounded-[2rem] border border-border shadow-2xl w-full max-w-xl mx-auto max-h-[90vh] md:max-h-[85vh] flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between p-5 md:p-6 border-b border-border bg-card/80 backdrop-blur-md sticky top-0 z-10 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg md:text-xl font-black text-foreground tracking-tight">Share Feedback</h2>
            <p className="text-xs md:text-sm text-muted-foreground font-medium">Your voice shapes our future</p>
          </div>
        </div>
        {onClose && (
